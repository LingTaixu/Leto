...
- [x] 7.3 全站测试：手动检查所有路由的视觉效果。**验证**：首页、关于页、博客页、Web3 首页、K 线页、转账页、存证页都显示硬边框 + 硬阴影。
- [x] 7.4 构建验证：`npm run build` 成功，无错误。**验证**：终端无 TypeScript 错误，`.next` 目录生成成功。（唯一警告为 middleware 已弃用，为 Next.js 16.3.5 引入的旧 warning，与本次改动无关）
